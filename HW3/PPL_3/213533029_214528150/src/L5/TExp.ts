...
;; <Pair>        ::= {left: TExp, right: TExp} // Pair(tvar, tvar)
...
export type CompoundTExp = ProcTExp | TupleTExp | PairExp;
...
// Pair exp definition
export type PairExp = { tag: "PairTExp"; left: TExp; right: TExp; };
export const makePairExp = (left: TExp, right: TExp): PairExp =>
    ({tag: "PairTExp", left: left, right: right});
export const isPairExp = (x: any): x is PairExp => x.tag === "PairTExp";
export const pairExpComponents = (pe: PairExp): TExp[] =>
    [pe.left, pe.right];    
...
*/
const parseCompoundTExp = (texps: Sexp[]): Result<ProcTExp | PairExp> => {
    
    const pair = texps.indexOf("Pair");
    const pos = texps.indexOf('->');
    
    return (pos === -1) ? 
           ((pair === -1) ? makeFailure(`Pair type expression must start with Pair - ${format(texps)}`) :
            (texps.length != 3) ? makeFailure(`Pair type expression must have exactly 2 elements - ${format(texps)}`) :
            (pair === 0) ? bind(mapResult(parseTExp ,texps.slice(1)), (list) => makeOk(makePairExp(list[0], list[1]))) :
            makeFailure(`Pair type expression must start with Pair - ${format(texps)}`)) :
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isPairExp(x) ? bind(unparseTExp(x.left), (left : string) =>
...
const matchTVarsInTE = <T1, T2>(te1: TExp, te2: TExp,
    ...
    (isAtomicTExp(te1) || isAtomicTExp(te2)) ?
        ((isAtomicTExp(te1) && isAtomicTExp(te2) && eqAtomicTExp(te1, te2)) ? succ([]) : fail()) :isPairExp(te1) && isPairExp(te2) ?
        matchTVarsInTEs(pairExpComponents(te1), pairExpComponents(te2), succ, fail) :
