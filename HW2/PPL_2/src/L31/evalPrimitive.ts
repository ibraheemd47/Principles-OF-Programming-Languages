...
const isDictPrim = (v: Value): boolean => {
   return isEmptySExp(v)? true
    : isCompoundSExp(v) &&
      isCompoundSExp(v.val1) &&
      isDictPrim(v.val2);
}

const getPrim = (args: Value[]): Result<SExpValue> => {
    if( args.length !== 2){
        return makeFailure('get expects 2 arguments');
    }
    const [dict, key] = args;
    if(!isCompoundSExp(dict) && !isEmptySExp(dict)){
        return makeFailure('get expects a list: ${format(dict)}');
    }
    if(!isSymbolSExp(key)){
        return makeFailure('get expects a symbol: ${format(key)}');
    }
    let curr: SExpValue = dict;
    while (isCompoundSExp(curr)){
        const element = curr.val1;
        if (isCompoundSExp(element) && isSymbolSExp(element.val1)){
            if (element.val1.val === key.val){
                return makeOk(element.val2);
            }
            
        }
        curr = curr.val2;
    }
    return makeFailure(`get: key ${key.val} not found in dict`);
};
const dictPrim = (args: Value[]): Result<SExpValue> => {
    if (args.length !== 1){
        return makeFailure('dict expects 1 argument');
    }
    if(!isCompoundSExp(args[0]) && isEmptySExp(args[0])){
        return makeFailure('dict expects a non-empty compound SExp');
    }
    let curr: SExpValue = args[0];
    const dictkeys: List<string> = [];
    while (isCompoundSExp(curr)){
        const element = curr.val1;
        if (isCompoundSExp(element) && isSymbolSExp(element.val1)){
            if (dictkeys.includes(element.val1.val)){
                return makeFailure(`dict expects unique keys, but got ${element.val1.val}`);
            }
            dictkeys.push(element.val1.val);
            curr = curr.val2;
        }
        else {
            return makeFailure('dict expects a non-empty compound SExp');
