...
import { DictExp, isCExp, isDictExp, isLetExp, makeDictExp, makeIfExp, makeVarRef } from "./L32-ast";
...
import { makeAppExp, makePrimOp } from "../L3/L3-ast";
...
export const evalDict= (exp: DictExp): Result<Value> => {
        const param: VarDecl = { tag: "VarDecl", var: "k" };
        const fallback: CExp = makeAppExp(makePrimOp("error"), [makeStrExp("Key not found")]);
        const body: CExp = exp.pairs.reduceRight((acc: CExp, entry) => {
            const keySymbol = entry.key;
            const condition = makeAppExp(makePrimOp("eq?"), [makeVarRef("k"), { tag: "LitExp", val: keySymbol }]);
            return makeIfExp(condition, entry.val, acc);
        }, fallback);
        return makeOk(makeClosure([param], [body]));
    };
