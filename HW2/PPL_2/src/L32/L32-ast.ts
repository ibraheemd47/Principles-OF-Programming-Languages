...
// L32
export type DictPair = {tag: "DictPair"; key: SymbolSExp; val: CExp; }
export type DictExp = {tag: "DictExp"; pairs: DictPair[]; }
...
export const makeLitExp = (val: SExpValue): LitExp =>
    ({tag: "LitExp", val: val});
// L32
export const makeDictPair = (key: SymbolSExp, val: CExp): DictPair =>
    ({tag: "DictPair", key: key, val: val});
export const makeDictExp = (pairs: DictPair[]): DictExp =>
    ({tag: "DictExp", pairs: pairs});
...
// L32
export const isDictPair = (x: any): x is DictPair => x.tag === "DictPair";
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isDictExp(x) || isDictPair(x);
...
const parseDictExp = (params: Sexp[]): Result<DictExp> => {
    const bad = params.find(p =>
        !isCompoundSexp(p) || p.length !== 2 || !isIdentifier(p[0])
      );
      if (bad)
        return makeFailure(`Bad dict binding: ${format(bad)}`);
    
      // extract keys (as SymbolSExp) and parse values
      const keys = params.map(p => makeSymbolSExp(p[0] as string));
      const valsR = mapResult(parseL32CExp, params.map(p => (p as [string, Sexp])[1]));
    
      return bind(valsR, (vals: CExp[]) => {
        const pairs = zipWith(makeDictPair, keys, vals);
        return makeOk(makeDictExp(pairs));
      });
...
const unparseDictPair = (dp: DictPair) : string =>
    `(${dp.key.val} ${unparseL32(dp.val)})`
...
export const unparseL32 = (exp: Program | Exp): string =>
    ...
    isDictExp(exp) ? `(dict ${exp.pairs.map((unparseDictPair)).join(" ")})` :
